...
select 
        ...
        dense_rank() over  ( partition by o.customer_id order by count( o.customer_id) desc  ) as dn
...
select a.customer_id, base.product_id, c.product_name
from
(select customer_id, max(ct) as mct from base group by customer_id) as a
left join base on a.customer_id = base.customer_id and a.mct = base.ct
left join Products c on base.product_id = c.product_id